...
public class BookAdminsServiceImpl extends ServiceImpl<BookAdminsMapper, BookAdmins>
    ...
    @Override
    public R<Map<String, Object>> getDashboardStatistics(Integer bookAdminId) {
        ...
        try {
            ...
            borrowCountWrapper.between(BooksBorrow::getBorrowDate, startOfDay, endOfDay);
            Long todayBorrow = booksBorrowService.count(borrowCountWrapper);
            statistics.put("todayBorrow", todayBorrow != null ? todayBorrow : 0);
            
            // 查询今日归还数量
            LambdaQueryWrapper<Violation> returnCountWrapper = new LambdaQueryWrapper<>();
            returnCountWrapper.between(Violation::getReturnDate, startOfDay, endOfDay);
            Long todayReturn = violationService.count(returnCountWrapper);
            statistics.put("todayReturn", todayReturn != null ? todayReturn : 0);
            
            // 查询待处理借阅（已借出未归还）
            LambdaQueryWrapper<BooksBorrow> pendingWrapper = new LambdaQueryWrapper<>();
            pendingWrapper.isNull(BooksBorrow::getReturnDate);
            Long pendingBorrow = booksBorrowService.count(pendingWrapper);
            statistics.put("pendingBorrow", pendingBorrow != null ? pendingBorrow : 0);
            
            // 查询逾期图书数量
            LocalDateTime now = LocalDateTime.now();
            LambdaQueryWrapper<BooksBorrow> overdueWrapper = new LambdaQueryWrapper<>();
            overdueWrapper.lt(BooksBorrow::getCloseDate, now)
                         .isNull(BooksBorrow::getReturnDate);
            Long overdueBooks = booksBorrowService.count(overdueWrapper);
            statistics.put("overdueBooks", overdueBooks != null ? overdueBooks : 0);
