...
@Tag(name = "系统管理员接口", description = "系统管理员功能接口")
public class AdminFunctionController {
    ...
    @Operation(summary = "根据图书 id 删除对应的图书")
    ...
    @Operation(summary = "根据图书 id 获得相对应的图书信息")
    ...
    @Operation(summary = "获取用户信息用于回显借阅证")
     ...
     * 获取系统管理员首页统计数据
     *
     * @return R 包含图书总数、读者数量、今日借阅、违章记录
     */
    @GetMapping("get_dashboard_statistics")
    @Operation(summary = "获取系统管理员首页统计数据")
    public R<Map<String, Object>> getDashboardStatistics() {
        return adminsService.getDashboardStatistics();
    }
     ...
     */
    @PostMapping("/updown")
    @Operation(summary = "从 Excel 批量导入图书")
     ...
     * @param genChartByAiRequest 用户输入信息 (分析目标，图标类型，名称),用户 id
     * @return R<BiResponse>
     */
    @PostMapping("/gen")
    @Operation(summary = "根据用户输入信息，生成图表")
     ...
     */
    @GetMapping("get_user_rank")
    @Operation(summary = "获取用户借阅排行榜")
    public R<List<UserRankDTO>> getUserRankList() {
        return usersService.getUserRankList();
    }

    /**
     * 获取图书热度排行榜（前 10 名）
     *
     * @return R<List<BookHotDTO>> 图书热度排行榜列表
     */
    @GetMapping("get_book_hot")
    @Operation(summary = "获取图书热度排行榜")
    public R<List<BookHotDTO>> getBookHotList() {
        return booksService.getBookHotList();
    }
}
